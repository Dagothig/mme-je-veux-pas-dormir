...
\usepackage[paperwidth=5.5in, paperheight=5in, hmargin=0.5in, vmargin=0.5in, outer=0.75in, inner=0.5in]{geometry}
\usepackage{layout}
...
\setstretch{1.0}
\definecolor{orange}{rgb}{0.75, 0.5, 0.25}

\newcommand{\imgpage}[1]{
    \tikz[remember picture,overlay]
    \node[inner sep=0pt] at (current page.center){\includegraphics[width=5.5in,height=5in]{#1}};
    \newpage
}
...
\fill[orange] (current page.north west) -- +(0.25in, 0.0in) -- +(0.25in, -\paperheight) -- (current page.south west);
\node[xshift=0.25in-5pt, yshift=0.125in, rotate=90, right] at (current page.south west){\textcolor{white}{MONSIEUR \textbf{MADAME}\hspace{1.575in}\textbf{DAGOTHIG}}};
\node[xshift=0.125in, yshift=-9pt-0.25in](name) at (current page.north){
    \textcolor{orange}{
        \huge
        \textbf{M}ME. \textbf{JE-VEUX-PAS-DORMIR}
    }
};
\node[text width=40pt, xshift=-56pt, yshift=0.25in](jeunesse) at (current page.south east){
...
\break
\scriptsize\textit{Madame Je-Veux-Pas-Dormir} \textcopyright\space 2027 diroum JEUNESSE (une société de Twado). Peu de droits réservés.
\newpage
\raggedright\normalsize
...
\txtpage{
Plus tard cette journée-là, Madame Je-Veux-Pas-Dormir décida de faire une marche dehors. Sur le
...
Pas de dépôt légal: 2 juin 2027\break
Définitivement hors la loi sur les publications destinées à la jeunesse.\break
Achevé d'imprimer par Msieur-qui-imprime-pour-Ariel à Longueuil.
\newpage
\normalsize
...
%4e de couverture
\begin{tikzpicture}[remember picture,overlay]
\fill[orange] (current page.north east) -- +(-0.25in, 0.0in) -- +(-0.25in, -\paperheight) -- (current page.south east);
